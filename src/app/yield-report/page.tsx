...
<body>
  ...
  // Batch-wise pipeline (HON→HL→VA) combined report — filter by batch id.
  // Scoped to the selected report date; reset the filter when the date changes.
  const [pipelineBatchFilter, setPipelineBatchFilter] = useState<string[]>([]);
  const [pipelinePrevDate, setPipelinePrevDate] = useState(selectedDate);
  if (selectedDate !== pipelinePrevDate) {
    setPipelinePrevDate(selectedDate);
    setPipelineBatchFilter([]);
  }
  ...
  // ── Batch-wise pipeline: HON→HL→VA joined by batch_id (selected date) ────────
  // Reuses the single-date `entries` (HON→HL) and `gradeVaEntries` (HL→VA) that
  // are already fetched — no extra queries. One row per batch with both stages,
  // plus end-to-end yield (VA/HON) and an HL reconciliation balance.
  const batchPipeline = useMemo(() => {
    interface PipeRow {
      ...
      graders: string[];
      honKgs: number;
      hlProduced: number;   // HL out of Stage 1 (yield_entries)
      hlConsumed: number;   // HL into Stage 2 (hl_va_entries)
      vaKgs: number;
      std1W: number; std1Base: number;   // kg-weighted Stage-1 standard
      std2W: number; std2Base: number;   // kg-weighted Stage-2 standard
    }
    const map = new Map<string, PipeRow>();
    const row = (id: string): PipeRow => {
      let r = map.get(id);
      if (!r) {
        r = {
          batchId: id, counts: [], varieties: [], grades: [], locations: [], graders: [],
          honKgs: 0, hlProduced: 0, hlConsumed: 0, vaKgs: 0,
          std1W: 0, std1Base: 0, std2W: 0, std2Base: 0,
        };
        map.set(id, r);
      }
      return r;
    ...
    // Stage 1: HON → HL
    entries.forEach((e) => {
      const r = row((e.batch_id || '—').trim());
      const hon = Number(e.hon_kgs) || 0;
      const hl = Number(e.hl_kgs) || 0;
      r.honKgs += hon;
      r.hlProduced += hl;
      push(r.counts, e.count_text);
      push(r.locations, e.location?.name);
      push(r.graders, e.grader_name);
      const s = lookupStandardYield(e.count_text);
      if (s !== null && hon > 0) { r.std1W += hon * s; r.std1Base += hon; }
    });
    ...
    gradeVaEntries.forEach((e) => {
      const r = row((e.batch_id || '—').trim());
      const hl = Number(e.hl_kgs) || 0;
      const va = Number(e.va_kgs) || 0;
      r.hlConsumed += hl;
      r.vaKgs += va;
      push(r.counts, e.count_text);
      push(r.varieties, e.variety);
      push(r.grades, e.grade || lookupHlVaCountRange(e.count_text) || '');
      push(r.locations, e.location?.name);
      push(r.graders, e.grader_name);
      const s = lookupHlVaStandardYield(e.count_text, e.variety);
      if (s !== null && hl > 0) { r.std2W += hl * s; r.std2Base += hl; }
    });
    ...
    return Array.from(map.values())
      .map((r) => ({
        ...
        graders: r.graders,
        honKgs: r.honKgs,
        hlProduced: r.hlProduced,
        hlConsumed: r.hlConsumed,
        vaKgs: r.vaKgs,
        stage1Yield: calculateYield(r.honKgs, r.hlProduced),                    // HL / HON
        stage2Yield: calculateYield(r.hlConsumed, r.vaKgs),                     // VA / HL
        overallYield: r.vaKgs > 0 ? calculateYield(r.honKgs, r.vaKgs) : null,   // VA / HON
        std1: r.std1Base > 0 ? r.std1W / r.std1Base : null,
        std2: r.std2Base > 0 ? r.std2W / r.std2Base : null,
        hlBalance: r.hlProduced - r.hlConsumed,
        hasStage1: r.honKgs > 0 || r.hlProduced > 0,
        hasStage2: r.hlConsumed > 0 || r.vaKgs > 0,
      }))
      .sort((a, b) => a.batchId.localeCompare(b.batchId, undefined, { numeric: true, sensitivity: 'base' }));
  }, [entries, gradeVaEntries]);

  const pipelineBatchOptions = useMemo(() => batchPipeline.map((r) => r.batchId), [batchPipeline]);

  const pipelineFiltered = useMemo(() => {
    if (!pipelineBatchFilter.length) return batchPipeline;
    return batchPipeline.filter((r) => pipelineBatchFilter.includes(r.batchId));
  }, [batchPipeline, pipelineBatchFilter]);

  const pipelineTotals = useMemo(() => {
    const t = pipelineFiltered.reduce(
      (acc, r) => {
        acc.hon += r.honKgs;
        acc.hlProduced += r.hlProduced;
        acc.hlConsumed += r.hlConsumed;
        acc.va += r.vaKgs;
        return acc;
      },
      { hon: 0, hlProduced: 0, hlConsumed: 0, va: 0 }
    );
    return {
      ...t,
      stage1Yield: calculateYield(t.hon, t.hlProduced),
      stage2Yield: calculateYield(t.hlConsumed, t.va),
      overallYield: t.va > 0 ? calculateYield(t.hon, t.va) : null,
      hlBalance: t.hlProduced - t.hlConsumed,
    };
  }, [pipelineFiltered]);

  // Colour a stage yield relative to its (kg-weighted) standard.
  const yieldTone = (y: number | null, std: number | null) => {
    if (y === null) return 'text-gray-400';
    if (std === null) return 'text-gray-900';
    return y >= std ? 'text-emerald-700' : 'text-rose-700';
  };
  ...
  return (
    <div className="animate-fade-in pb-20 lg:pb-6">
      ...
      <div className="px-4 mt-2 space-y-6">
        ...
        {/* ─── Batch-wise Pipeline (HON → HL → VA) ─────────────────────── */}
        <div className="space-y-4 mb-8">
          <div className="pt-2 pb-1">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">
              Batch-wise Pipeline
              <span className="ml-2 text-sm font-semibold text-gray-400">HON → HL → VA</span>
            </h2>
            <p className="text-sm text-gray-500 mt-0.5">
              Each batch&apos;s full journey for {selectedDate} — both processing stages joined by Batch ID.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            ) : batchPipeline.length === 0 ? (
              <div className="p-8 text-center">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-indigo-50 mb-3 text-indigo-600 text-xl">🔗</div>
                <p className="text-sm font-semibold text-gray-900">No Data Available</p>
                <p className="text-sm text-gray-500 mt-1">No HON→HL or HL→VA entries for {selectedDate}.</p>
              </div>
            ) : pipelineFiltered.length === 0 ? (
              <div className="p-8 text-center">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-indigo-50 mb-3 text-indigo-600 text-xl">🔍</div>
                <p className="text-sm font-semibold text-gray-900">No Matches Found</p>
                <p className="text-sm text-gray-500 mt-1">No batches match the selected Batch ID filter.</p>
                <button
                  type="button"
                  onClick={() => setPipelineBatchFilter([])}
                  className="mt-3 text-sm font-semibold text-indigo-600 hover:underline"
                >
                  Clear filter
                </button>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    ...
                    <tr className="bg-gray-50 border-b border-gray-100 dark:border-gray-800">
                      <th className="px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap sticky left-0 bg-gray-50 dark:bg-gray-900 z-10 shadow-[1px_0_0_0_#f3f4f6] dark:shadow-[1px_0_0_0_#374151]">
                        <ColumnFilter
                          ...
                          selected={pipelineBatchFilter}
                          onChange={setPipelineBatchFilter}
                        />
                      </th>
                      <th className="px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Counts</th>
                      ...
                      <th className="px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap text-right">HON (KGS)</th>
                      <th className="px-4 py-3 text-[10px] font-semibold text-teal-500 uppercase tracking-wider whitespace-nowrap text-right">HL Prod</th>
                      <th className="px-4 py-3 text-[10px] font-semibold text-teal-500 uppercase tracking-wider whitespace-nowrap text-right" title="Stage 1 yield: HL ÷ HON">S1 Yield</th>
                      <th className="px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap text-right">HL Used</th>
                      <th className="px-4 py-3 text-[10px] font-semibold text-indigo-500 uppercase tracking-wider whitespace-nowrap text-right">VA (KGS)</th>
                      <th className="px-4 py-3 text-[10px] font-semibold text-indigo-500 uppercase tracking-wider whitespace-nowrap text-right" title="Stage 2 yield: VA ÷ HL">S2 Yield</th>
                      <th className="px-4 py-3 text-[10px] font-semibold text-emerald-600 uppercase tracking-wider whitespace-nowrap text-right" title="End-to-end yield: VA ÷ HON">Overall</th>
                      <th className="px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap text-right" title="HL produced (Stage 1) − HL used (Stage 2). A nonzero balance is normal across days (carried forward / drawn from stock).">HL Bal</th>
                      <th className="px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Location</th>
                      <th className="px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Grader</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {pipelineFiltered.map((r) => {
                      const bal = r.hlBalance;
                      const balTone = !r.hasStage1 || !r.hasStage2
                        ? null
                        : Math.abs(bal) < 0.001
                      ...
                      return (
                        <tr key={r.batchId} className="hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-colors group">
                          <td className="px-4 py-3 text-sm font-medium text-gray-900 whitespace-nowrap sticky left-0 bg-white dark:bg-gray-900 group-hover:bg-gray-50 dark:group-hover:bg-gray-800/80 z-10 shadow-[1px_0_0_0_#f3f4f6] dark:shadow-[1px_0_0_0_#374151]">{r.batchId}</td>
                          <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{r.counts.join(', ') || '-'}</td>
                          <td className="px-4 py-3 text-sm whitespace-nowrap">
                            {r.varieties.length > 0 ? (
                              <div className="flex flex-wrap gap-1">
                                {r.varieties.map((v) => (
                                  <span key={v} className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700">{v}</span>
                                ))}
                              </div>
                            ) : <span className="text-gray-400">-</span>}
                          </td>
                          <td className="px-4 py-3 text-sm font-semibold text-indigo-700 dark:text-indigo-400 whitespace-nowrap">{r.grades.join(', ') || '-'}</td>
                          <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap text-right font-medium">{r.honKgs > 0 ? r.honKgs.toFixed(3) : '-'}</td>
                          <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap text-right font-medium">{r.hlProduced > 0 ? r.hlProduced.toFixed(3) : '-'}</td>
                          <td className={`px-4 py-3 text-sm whitespace-nowrap text-right font-bold ${yieldTone(r.stage1Yield, r.std1)}`} title={r.std1 !== null ? `Std ${r.std1.toFixed(2)}%` : undefined}>{r.stage1Yield !== null ? `${r.stage1Yield.toFixed(2)}%` : '-'}</td>
                          <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap text-right font-medium">{r.hlConsumed > 0 ? r.hlConsumed.toFixed(3) : '-'}</td>
                          <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap text-right font-medium">{r.vaKgs > 0 ? r.vaKgs.toFixed(3) : '-'}</td>
                          <td className={`px-4 py-3 text-sm whitespace-nowrap text-right font-bold ${yieldTone(r.stage2Yield, r.std2)}`} title={r.std2 !== null ? `Std ${r.std2.toFixed(2)}%` : undefined}>{r.stage2Yield !== null ? `${r.stage2Yield.toFixed(2)}%` : '-'}</td>
                          <td className="px-4 py-3 text-sm whitespace-nowrap text-right font-bold text-emerald-700">{r.overallYield !== null ? `${r.overallYield.toFixed(2)}%` : '-'}</td>
                          <td className="px-4 py-3 text-sm whitespace-nowrap text-right">
                            {balTone ? (
                              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${balTone}`}>{bal > 0 ? '+' : ''}{bal.toFixed(3)}</span>
                            ) : <span className="text-gray-400">-</span>}
                          </td>
                          <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{r.locations.join(', ') || '-'}</td>
                          <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{r.graders.join(', ') || '-'}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                  <tfoot>
                    <tr className="bg-indigo-50 dark:bg-indigo-900/30 border-t-2 border-indigo-100 dark:border-indigo-800">
                      <td className="px-4 py-3 text-sm font-bold text-indigo-900 dark:text-indigo-300 whitespace-nowrap sticky left-0 bg-indigo-50 dark:bg-gray-900 z-10 shadow-[1px_0_0_0_#e0e7ff] dark:shadow-[1px_0_0_0_#3730a3]">TOTALS</td>
                      <td className="px-4 py-3 text-sm text-indigo-800 dark:text-indigo-300 whitespace-nowrap">{pipelineFiltered.length} batches</td>
                      <td className="px-4 py-3"></td>
                      <td className="px-4 py-3"></td>
                      <td className="px-4 py-3 text-sm font-bold text-indigo-900 dark:text-indigo-300 whitespace-nowrap text-right">{pipelineTotals.hon > 0 ? pipelineTotals.hon.toFixed(3) : '-'}</td>
                      <td className="px-4 py-3 text-sm font-bold text-indigo-900 dark:text-indigo-300 whitespace-nowrap text-right">{pipelineTotals.hlProduced > 0 ? pipelineTotals.hlProduced.toFixed(3) : '-'}</td>
                      <td className="px-4 py-3 text-sm font-bold text-indigo-900 dark:text-indigo-300 whitespace-nowrap text-right">{pipelineTotals.stage1Yield !== null ? `${pipelineTotals.stage1Yield.toFixed(2)}%` : '-'}</td>
                      <td className="px-4 py-3 text-sm font-bold text-indigo-900 dark:text-indigo-300 whitespace-nowrap text-right">{pipelineTotals.hlConsumed > 0 ? pipelineTotals.hlConsumed.toFixed(3) : '-'}</td>
                      <td className="px-4 py-3 text-sm font-bold text-indigo-900 dark:text-indigo-300 whitespace-nowrap text-right">{pipelineTotals.va > 0 ? pipelineTotals.va.toFixed(3) : '-'}</td>
                      <td className="px-4 py-3 text-sm font-bold text-indigo-900 dark:text-indigo-300 whitespace-nowrap text-right">{pipelineTotals.stage2Yield !== null ? `${pipelineTotals.stage2Yield.toFixed(2)}%` : '-'}</td>
                      <td className="px-4 py-3 text-sm font-bold text-emerald-700 whitespace-nowrap text-right">{pipelineTotals.overallYield !== null ? `${pipelineTotals.overallYield.toFixed(2)}%` : '-'}</td>
                      <td className="px-4 py-3 text-sm font-bold text-indigo-900 dark:text-indigo-300 whitespace-nowrap text-right">{pipelineTotals.hlBalance.toFixed(3)}</td>
                      <td className="px-4 py-3"></td>
                      <td className="px-4 py-3"></td>
                    </tr>
                  </tfoot>
